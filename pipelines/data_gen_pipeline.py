...
Data generation pipeline for the ProteinStructure ESP project.

Runs all data-generation steps sequentially for each selected protein fragment:
    ...
    5. Sample ESP onto the PQR mesh (interpolated)
...
Requires the data-generation conda environment (APBS, MSMS, PDB2PQR).

Usage:
    python pipelines/data_gen_pipeline.py --id-file data/protein_ids.txt
    python pipelines/data_gen_pipeline.py --id-file data/protein_ids.txt --data-root /path/to/data
    python pipelines/data_gen_pipeline.py --id-file data/protein_ids.txt --workers 4
"""
...
def _run_protein(protein_id: str, data_root: Path, pipeline_log, keep_dx: bool = False) -> dict:
    """
    Run all data-generation steps for a single protein fragment.
...
def main():
    parser = argparse.ArgumentParser(
        description="Data generation pipeline: download → PDB2PQR → APBS → mesh → ESP → evaluate."
    )
