...
Visualize the ESP surface for a single protein.
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize the ESP surface for a single protein."
    )
    ...
    try:
        plot_esp(args.protein_id, data_root, clim=clim)
    except FileNotFoundError as e:
        log.error("Missing files: %s", e)
        print(f"✗  {args.protein_id}  —  missing ESP file (run scripts 01-04 first)")
